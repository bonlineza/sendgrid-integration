...
class SendGrid
{
    ...
    $secrets_json = json_decode($secrets_str);
	    ...
	    } else if (!isset($secrets_json['SENDGRID_API_USER']) || !isset($secrets_json['SENDGRID_API_PASSWORD'])) {
	      throw new Exception('Sendgrid credentials missing from secrets');
	    } else {
				$this->apiUser = $secrets_json['SENDGRID_API_USER'];
				$this->apiKey = $secrets_json['SENDGRID_API_PASSWORD'];
...
$test = new SendGrid("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");
